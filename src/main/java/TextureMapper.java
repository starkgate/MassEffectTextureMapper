...
public class TextureMapper extends Application {
	...
	public void start(Stage primaryStage) throws IOException {
		...
		String[] spreadsheetIds = { "1Gvnz_trNOUgW6CSI3eeEYk7SvqsSklIgVuoHDlmw8e8",
				"1VZKgxDAfASaZVcNHbeIFEClxRxn3v2-sNwSOEvgYHrs" }; // texture map, authors
		String textureMapRange = "Texture_Map_Full!A2:K";
		String me2AuthorsRange = "ALOT_ME2!A4:B";
		String me3AuthorsRange = "ALOT_ME3!A4:B";

		ValueRange textureMap = service.spreadsheets().values().get(spreadsheetIds[0], textureMapRange).execute();
		// Build database of textures
		List<List<Object>> textureMapValues = textureMap.getValues();

		ValueRange authorsME2 = service.spreadsheets().values().get(spreadsheetIds[1], me2AuthorsRange).execute();
		List<List<Object>> textureAuthorsME2 = authorsME2.getValues();

		ValueRange authorsME3 = service.spreadsheets().values().get(spreadsheetIds[1], me3AuthorsRange).execute();
		...
		portSoloTextures.setTooltip(new Tooltip(
				...
				if (crcs.length() != 0) {
					...
					if (selectedGame != null) {
						...
						for (String sourceHash : crcList) {
							List<List<String>> crcDuplicates = database.search(sourceHash, selectedGame,
									getPortSoloTextures(), getMatchAuthor());
							if (crcDuplicates != null) { // if texture can't be found in the texture map, skip it
								for (List<String> crc : crcDuplicates) {
									...
									duplicateText.appendText(sourceHash + " -> " + destinationHash + " (" + textureClass
											+ ") " + author); // crc and class
